...
last_changed: 2021-08-26 21:05 UTC
...
## Intergration to \code{nilearn}

The team discussed the idea to migrate the full code base to \code{nilearn} when the package is in beta.
Adding \code{load\_confounds} to a high profile project such as \code{nilearn} will maximise the benifit to a wider fMIR community.
As an extension of Brainhack Project, Hao-Ting Wang and Pierre Bellec prepared material to discussed with developers of \code{nilearn} at their code sprint.
To facilitate the inegration, Hao-Ting Wang added \code{sample\_mask} to support volume-sensoring based scrubbing and improve the \code{nilearn} \code{NiftiMasker} and \{signal.clean} to ensure correct application of scrubbing.
The number of predefined strategy has also been trimmed down to four basics: \code{simple}, \code{scrubbing}, \code{compcor}, and \code{ica\_aroma}.

# Results

\code{load\_confounds} has now covered all possible approacies from fMRIPrep outputs and most of the strategies tested in Ciric et al. \cite{ciric:2017}.
...
\begin{itemize}
  ...
  \item \code{compcor} \cite{compcor:2007}: the results of a PCA applied on a mask based on either anatomy or temporal variance (\code{temporal}). For aCompCor, one can choose either separate or combined WM and CSF compartments (\code{anat\_combined}, \code{anat\_separated}), as well as choosing to extract components contributing 50\% of variance or a set number of components (typically 5).
...
\code{load\_confounds} provides four preset strategies for the ease of access. The following table shows an over view of noise components applied to each preset strategy:
...
\begin{tabular}{ | l | c | l | l | l | l | l | l |  }
  ...
  \hline
  \code{simple}     & x                 & \code{full*}  & \code{basic*}  & \code{None*}  & N/A                   & N/A               & N/A           \\
  \code{scrubing}   & x                 & \code{full*}  & \code{full*}   & \code{None*}  & N/A                   & N/A               & \code{5\*}    \\
  \code{compcor}    & x                 & \code{full*}  & N/A            & N/A           & \code{anat\_combined} & N/A               & N/A           \\
  \code{ica\_aroma} & x                 & N/A           & \code{basic*}  & \code{None*}  & N/A                   & \code{full}       & N/A           \\
  \hline
\end{tabular}
